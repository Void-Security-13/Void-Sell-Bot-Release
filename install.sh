...
# ایجاد دستور سراسری Void برای باز کردن پنل مدیریتی از هر مسیر
$SUDO tee /usr/local/bin/Void >/dev/null <<EOF
#!/usr/bin/env bash
cd "${REPO_DIR}" || exit 1
exec bash "${REPO_DIR}/void-bot.sh"
EOF
...
echo "پنل مدیریت:     از هر مسیر فقط دستور Void را اجرا کنید."
